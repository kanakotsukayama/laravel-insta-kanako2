...
class ProfileController extends Controller
{
    ...
    public function update(Request $request){
        ...
        $request->validate([
            ...
            'email'=>'required|max:50|email|unique:users,email,'.Auth::user()->id,  //email -> email format
            //when adding new record, unique:<table name>,<column name>
            //when updating, unique:<table>,<column>.<id>introduction
            'introduction' => 'max:100'
        ]);

        //update name, email, introduction
        $user_a = $this->user->findOrFail(Auth::user()->id);  ///we are editing a profile who is logged in
