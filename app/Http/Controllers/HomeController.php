...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        if($request->search){ //if search
            $home_posts = $this->post->where('description','like','%'. $request->search. '%')->latest()->get();  //SELECT*...WHERE description LIKE(= but if you use% you user like) '%searchword%'
        }else
        {
        ...
        return view('user.home')->with('all_posts', $home_posts)
                                ->with('suggested_users', $this->getSuggestedUsers())->with('search',$request->search);
    }
