          ...
          <div class="col-auto">
              ...
              @if($follow->followed->id != Auth::user()->id)
                  @if($follow->followed->isFollowed())
                   ...
                   @else
                   {{-- follow --}}
                      <form action="{{route('follow.store', $follow->id)}}" method="post">
                      @csrf
                      <button type="submit" class="border-0 shadow-none p-0 bg-stansparent text-primary">Follow</button>
...
@else
<h4 class="h5 text-muted text-center">No Following anyone yet.</h4>
@endif
